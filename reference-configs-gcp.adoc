...
Dans les déploiements à nœud unique, à partir de Cloud Volumes ONTAP 9.18.1, si vous sélectionnez les variantes SSD locales (LSSD) des machines virtuelles C3, le cache Flash est activé par défaut. Vous ne pouvez pas changer de variante de machine virtuelle C3 LSSD dans votre déploiement Cloud Volumes ONTAP, même si la variante cible prend en charge le cache Flash.
