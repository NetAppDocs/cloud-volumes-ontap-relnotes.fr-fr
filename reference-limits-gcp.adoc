...
Non, ce n'est pas le cas. Les données d'une paire HA sont mises en miroir de manière synchrone entre les nœuds afin que les données soient disponibles en cas de panne dans Google Cloud. Par exemple, si vous achetez un disque de 8 To sur le nœud A, la console alloue également un disque de 8 To sur le nœud B qui est utilisé pour les données en miroir. Bien que 16 To de capacité soient configurés, seuls 8 To sont comptabilisés dans la limite de licence.
